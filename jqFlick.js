(function($){
	...
	$.fn.jqFlick = function(options){
		...
		var YQL = 'http://query.yahooapis.com/v1/public/yql?q={QUERY}&format=json&callback=?',
						query = "SELECT * FROM flickr.photos.search WHERE api_key='{KEY}' AND text='{PHOTOSET}' LIMIT 10";
						
			//query1 = "SELECT * FROM flickr.photosets.photos({MAX}) WHERE photoset_id='{PHOTOSET}'"+
				//" AND api_key='{KEY}'";
